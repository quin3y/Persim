	...
	void Start()
		...
		void Update()
		{
			...
			if (Playlist.playlist.Count() == 0) {
				waitForNewActivity = true;
			}
			/*if (Playlist.playlist.Count() > 0&& waitForNewActivity) {
				Playlist.PlayActivity (NameList.IndexOf (toDoList [0]));
				Playlist.playlist.Pop ();
				waitForNewActivity = false;
			}*/
		}
			
	void OnGUI () {
		// Logic for time in the top left
		string timer = String.Format("{0:00}:{1:00}", Mathf.Floor(time/60), time % 60);
		hours = Mathf.Floor (time / 3600) + 7;
		string hour = String.Format ("{0:00}", hours);
		...
		if (!menu) {  //Only show this GUI if the main menu is not up
			/*if (Playlist.playlist.Count () == 0) {
				...
				GUI.Button (new Rect (Screen.width-253, 15, 242, 30), hours + ":" + timer + " AM  " + currActivity, "nothing");
				...
				if (!play) {
					if (GUI.Button (new Rect (Screen.width-149, 37, 40, 27), "", "pause")) {
				...
				if (GUI.Button (new Rect (Screen.width-110, 37, 40, 27), "", "fastf")) {
					if (Time.timeScale <= 1024) {
						Time.timeScale = Time.timeScale *2;
					}
				}
				if (GUI.Button (new Rect (Screen.width-188, 37, 40, 27), "", "slowd")) {
					Time.timeScale = Time.timeScale/(Time.timeScale *2);
				}
