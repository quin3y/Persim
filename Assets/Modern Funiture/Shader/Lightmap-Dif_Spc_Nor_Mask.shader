    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                float2 node_724 = i.uv0;
                clip(tex2D(_Mask,TRANSFORM_TEX(node_724.rg, _Mask)).r - 0.5);
                i.normalDir = normalize(i.normalDir);
                float3x3 tangentTransform = float3x3( i.tangentDir, i.binormalDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_724.rg, _Normal))).rgb;
...
////// Specular:
                ...
                finalColor += diffuseLight * ((_LightmapBrightness*tex2Dlod(_LightMap,float4(TRANSFORM_TEX(node_686.rg, _LightMap),0.0,node_686.r)).rgb)*tex2D(_MainTex,TRANSFORM_TEX(node_724.rg, _MainTex)).rgb);
                finalColor += specular;
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                clip(tex2D(_Mask,TRANSFORM_TEX(node_725.rg, _Mask)).r - 0.5);
                i.normalDir = normalize(i.normalDir);
                float3x3 tangentTransform = float3x3( i.tangentDir, i.binormalDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_725.rg, _Normal))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                ...
                float3 diffuseLight = diffuse;
                float2 node_686 = i.uv1;
                finalColor += diffuseLight * ((_LightmapBrightness*tex2Dlod(_LightMap,float4(TRANSFORM_TEX(node_686.rg, _LightMap),0.0,node_686.r)).rgb)*tex2D(_MainTex,TRANSFORM_TEX(node_725.rg, _MainTex)).rgb);
                finalColor += specular;
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                clip(tex2D(_Mask,TRANSFORM_TEX(node_726.rg, _Mask)).r - 0.5);
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                float2 node_727 = i.uv0;
                clip(tex2D(_Mask,TRANSFORM_TEX(node_727.rg, _Mask)).r - 0.5);
